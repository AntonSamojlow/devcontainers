...
# --- Install *specified* version of gcc from https://packages.ubuntu.com or https://launchpad.net/~ubuntu-toolchain-r/+archive/ubuntu/ppa ---
ARG GCC_VERSION=13
...
RUN Get-Command "g++-$env:GCC_VERSION" | Write-Host
RUN Get-Command "gcc-$env:GCC_VERSION" | Write-Host
...
RUN update-alternatives --install /usr/bin/clang clang "/usr/bin/clang-$env:GCC_VERSION" 20
RUN update-alternatives --install /usr/bin/clang++ clang++ "/usr/bin/clang++-$env:GCC_VERSION" 20
